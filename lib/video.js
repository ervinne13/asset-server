...
// Skip the first n frames of a video (frame-accurate, stream-copy).
// Used to strip the overlap frames prepended by the extended SCAIL workflow.
async function trimFirstNFrames(input, output, n, fps) {
  await ffmpeg(['-i', input, '-ss', String(n / fps), '-c', 'copy', output]);
}
