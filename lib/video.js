...
// Skip the first n frames of a video. Re-encodes so the output starts on a
// keyframe and is playable. Used to strip overlap frames from extended SCAIL segments.
async function trimFirstNFrames(input, output, n) {
  await ffmpeg([
    '-i', input,
    '-vf', `select='gte(n,${n})',setpts=PTS-STARTPTS`,
...
async function probeFps(filePath) {
  ...
  const raw = out.trim(); // e.g. "24000/1001" or "30/1"
  const [num, den] = raw.split('/').map(Number);
  const fps = den && den > 0 ? num / den : num;
  return Number.isFinite(fps) && fps > 0 ? fps : null;
}

module.exports = { extractLastFrame, probeDuration, probeHasAudio, probeFps, joinVideos, trimFirstNFrames };
